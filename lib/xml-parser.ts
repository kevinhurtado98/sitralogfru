...
export function parseFacturaXML(xmlContent: string): FacturaXMLData {
  ...
  if (!invoice) throw new XMLParseError('No se encontró el nodo Invoice en el XML')

  // ── Serie-Número ────────────────────────────────────────────────────────────
  const serieNumeroRaw = getString(invoice, 'cbc:ID') ?? getString(invoice, 'ID')
  if (!serieNumeroRaw) throw new XMLParseError('No se encontró el ID de la factura')
  ...
  // ── Fechas ──────────────────────────────────────────────────────────────────
  const fechaEmisionStr = getString(invoice, 'cbc:IssueDate') ?? getString(invoice, 'IssueDate')
  if (!fechaEmisionStr) throw new XMLParseError('No se encontró la fecha de emisión')
  ...
  // ── Monto ───────────────────────────────────────────────────────────────────
  const lmt = getObj(invoice, 'cac:LegalMonetaryTotal') ?? getObj(invoice, 'LegalMonetaryTotal')
  const montoRaw = getNumber(lmt ?? {}, 'cbc:PayableAmount') ?? getNumber(lmt ?? {}, 'PayableAmount')
  if (!montoRaw || montoRaw <= 0) throw new XMLParseError('No se encontró el monto a pagar')

  // ── Proveedor ───────────────────────────────────────────────────────────────
  const supplier =
    getObj(invoice, 'cac:AccountingSupplierParty') ??
    getObj(invoice, 'AccountingSupplierParty')
  const party = getObj(supplier ?? {}, 'cac:Party') ?? getObj(supplier ?? {}, 'Party')
  const legalEntity = getObj(party ?? {}, 'cac:PartyLegalEntity') ?? getObj(party ?? {}, 'PartyLegalEntity')
  const proveedor = getString(legalEntity ?? {}, 'cbc:RegistrationName') ??
    getString(legalEntity ?? {}, 'RegistrationName') ??
    'Desconocido'

  // RUC: busca en PartyIdentification con schemeID="6"
  const partyId = getObj(party ?? {}, 'cac:PartyIdentification') ?? getObj(party ?? {}, 'PartyIdentification')
  const idNode = (partyId as Record<string, unknown>)?.['cbc:ID'] ?? (partyId as Record<string, unknown>)?.['ID']
...
function extractFechaVencimiento(invoice: Record<string, unknown>): Date | null {
  // Opción 1: DueDate directo
  const directDue = getString(invoice, 'cbc:DueDate') ?? getString(invoice, 'DueDate')
...
function extractRUC(idNode: unknown): string {
  ...
  if (typeof idNode === 'string' || typeof idNode === 'number') return String(idNode)
...
function parseSerieNumero(id: string): [string, string] {
  ...
  if (dashIdx > 0) return [clean.slice(0, dashIdx).toUpperCase(), clean.slice(dashIdx + 1)]
  return ['F001', clean]
}
