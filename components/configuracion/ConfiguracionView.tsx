...
export function ConfiguracionView({
  ...
  return (
    <>
      ...
      <div className="dc">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, paddingBottom: 9, borderBottom: '1px solid var(--bl)' }}>
          <span className="ss" style={{ margin: 0, padding: 0, border: 'none' }}>Áreas y responsables</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 400 }}>
            ...
            <tbody>
              {responsables.length === 0 && (
                <tr><td colSpan={3} style={{ padding: '16px 11px', color: 'var(--t3)', fontSize: 13 }}>No hay responsables registrados.</td></tr>
              )}
              {responsables.map(r => (
                <tr key={r.id} style={{ borderBottom: '1px solid var(--bl)' }}>
                  <td style={{ padding: '8px 11px' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '2px 8px', borderRadius: 20, background: r.areaColor, color: r.areaTc, fontSize: 12, fontWeight: 500 }}>
                      {r.areaNombre}
                    </span>
                  </td>
                  <td style={{ padding: '8px 11px' }}>
      ...
      <div className="dc">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, paddingBottom: 9, borderBottom: '1px solid var(--bl)' }}>
          <span className="ss" style={{ margin: 0, padding: 0, border: 'none' }}>Usuarios del sistema</span>
      ...
      <div className="dc">
        ...
        <div style={{ marginTop: 13, display: 'flex', justifyContent: 'flex-end' }}>
          <button className="btn btn-p"><IconDeviceFloppy size={13} /> Guardar</button>
        </div>
      </div>
    </>
  )
